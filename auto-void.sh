...
xbps-install -y linux6.6
xbps-install -y linux6.6-headers
xbps-reconfigure -f linux-$(uname -r)

# Setting up seatd
echo "Installing seatd & configuring dumb_runtime_dir"
usermod -aG input,video,audio sm
...
xbps-install -y wayland dbus git curl base-devel wget nano docker docker-compose docker-cli lazydocker lazygit gcc meson sassc
...
xbps-install -y noto-fonts-emoji noto-fonts-ttf font-hack-ttf font-awesome nerd-fonts
...
echo "/home/sm 192.168.100.15/255.255.255.0(rw,async,no_subtree_check,anonuid=1000,anongid=1000) 192.168.100.11/255.255.255.0(rw,async,no_subtree_check,anonuid=1000,anongid=1000)" >> /etc/exports
